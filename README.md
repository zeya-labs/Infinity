...
贡献和本地检查流程见 [CONTRIBUTING.md](CONTRIBUTING.md)，工程维护约定见
[docs/engineering.md](docs/engineering.md)，安全问题报告流程见 [SECURITY.md](SECURITY.md)。
...
提交 normal 训练、TUI 或脚本入口相关改动前，建议运行轻量检查：

```bash
scripts/check.sh
```

该命令会执行关键 Python 文件语法检查、`tests/` 下的单元测试，以及 normal 训练 shell 入口的 `bash -n` 语法检查。
...
docker run --gpus all -it --name my-container -v "$PWD":/workspace my-flash-attn-env
```
...
本仓库也提供 [CITATION.cff](CITATION.cff)，GitHub 和常见引用工具可以直接读取。
...
本项目源代码使用 MIT License，详情见 [LICENSE](LICENSE)。外部权重、数据集和服务的许可边界见

...
def build_train_normal(values: dict[str, str]) -> list[str]:
    ...
    if values["hypersim_filter_depth_nan"].lower() in {"1", "yes", "true", "y"}:
        cmd.append("--hypersim-filter-depth-nan")
...
TASKS: list[Task] = [
    ...
    Task(
        ...
        [
            ...
            Field("word_head_min_lr", "Word/head min LR", "1e-6"),
            Field("image_word_lr", "Image word LR", "5e-5"),
            Field("image_word_min_lr", "Image word min LR", "2.5e-6"),
            Field("normal_task_lr", "Normal task LR", "1e-4"),
            Field("normal_task_min_lr", "Normal task min LR", "5e-6"),
